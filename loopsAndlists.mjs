...
let toungeTwister = " If the zookeeper had to choose, would the zookeeper choose to use the moose the zookeeper uses, or would the zookeeper choose to choose a new moose to use?"
let listOfwords = toungeTwister.split(` `);
let wordsDuplicated = [];
for (let i = 0; i < listOfwords.length; i++) {
    if (!wordsDuplicated.includes(listOfwords[i])) {
        wordsDuplicated.push(listOfwords[i])
    }
};
...
for (let i = 0; i < drawing.length; i++) {
    arraytoString += drawing[i];
    if (i < drawing.length - 1) {
        arraytoString += `,`;
    }
}
...
console.log(lostNumbers(digits));
